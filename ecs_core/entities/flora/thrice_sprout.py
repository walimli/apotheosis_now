"""Thrice sprout entity definition."""
...
def _default_config() -> SproutConfig:
    return SproutConfig(
        sprite_path=str((SPRITE_ROOT / "flower_3.png").resolve()),
        registry_id="thrice_sprout",
        next_entity_id="quarce_sprout",
    )


def spawn_thrice_sprout(
    world: World,
    entity_manager: EntityManager,
    *,
    config: Optional[SproutConfig] = None,
) -> Entity:
    """Create and register the thrice sprout entity inside the ECS world."""

    return spawn_sprout_entity(world, entity_manager, config=config or _default_config())


evolvable_registry.register_factory("thrice_sprout", spawn_thrice_sprout)


__all__ = ["spawn_thrice_sprout", "SproutConfig"]
