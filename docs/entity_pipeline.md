...
- `spawn_per`: `"island" | "tile" | "void" | "chunk" | "event"`.
  ...
  - `chunk`: process the entire chunk as the region; random tiles (land or void) are selected so scatterable loot like treasure cores can drop anywhere.
...
4. Spawn rules that target `world_start` evaluate against the chunk's tile data: the service finds islands, picks eligible tiles, and calls `_spawn_entity`. When `spawn_per` is set to `"chunk"`, the factory simply samples random coordinates across the entire chunk, which is ideal for high-volume scatter drops like `treasure_core`.
...
### Treasure Core Example

- **Spawn config**: `data/entities/treasure_core_spawn.json` introduces a `world_start` rule with `spawn_per: "chunk"` and `spawn_number: "randomint(5,20)"`. Every generated chunk therefore scatters 5–20 treasure cores anywhere on the grid (land or void).
- **Entity factory**: `ecs_core/entities/treasure_core.py` registers `treasure_core` with the evolvable registry. Each core is stationary, has 3 HP, WALL-layer collision, idle animation, and a `Drops` payload that grants XP plus a `treasure_coin`.
- **Drop pipeline**: when the core dies, `DropsSystem` spawns the configured coin entities. Standard coins still route through `RenderableEntityComponent`, but their `size` is now `(32,32)` for better readability. The moon coin drop uses `spawn_treasure_coin_drop`, which animates the 32px spritesheet so the pickup matches the new footprint.
- **Factory import**: `MonsterFactoryService._ensure_entity_factories_loaded()` now imports the treasure-core module so JSON lookups resolve automatically once the spawn file exists.
