...
class PlayerAnimator:
    def __init__(self) -> None:
        ...
        self._pick_equipped: bool = False
    ...
    def set_pick_equipped(self, equipped: bool) -> None:
        ...
        if equipped == self._pick_equipped:
            return
        self._pick_equipped = equipped
        if self._action_name is None:
            state_name = self._resolve_state(f"{self._base_action}_{self._last_direction}")
    ...
    def update(self, dt: float) -> None:
        ...
        state_name = self._resolve_state(f"{self._base_action}_{direction}")
        if not state_name:
            return
        if self._pick_equipped and not state_name.endswith("_pick"):
            pick_candidate = self._resolve_state(f"{self._base_action}_{direction}_pick", prefer_pick=True)
            if pick_candidate:
                state_name = pick_candidate
    ...
    def _resolve_state(self, name: str, prefer_pick: Optional[bool] = None) -> Optional[str]:
        if name in self.states:
            return name

        explicit_pick: Optional[bool] = None
        base = name
        ...
        prefer = (
            ...
            else (self._pick_equipped if prefer_pick is None else prefer_pick)
        )
        return self._resolve_components(action, direction, prefer)

    def _resolve_components(self, action: str, direction: str, prefer_pick: Optional[bool]) -> Optional[str]:
        base_name = f"{action}_{direction}"
        candidates = []
        if prefer_pick is True:
            candidates.append(f"{base_name}_pick")
            candidates.append(base_name)
        elif prefer_pick is False:
            candidates.append(base_name)
        else:
            candidates.append(f"{base_name}_pick")
            candidates.append(base_name)
