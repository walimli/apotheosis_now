...
class NotificationScreenResult(Enum):
    NONE = "none"
    PREVIOUS = "previous"
    CURRENT = "current"
    CLOSE = "close"
...
class NotificationScreen(JournalScreen):
    """Displays the current notification with navigation controls."""
    ...
    def handle_event(self, event) -> NotificationScreenResult:
        if not self.is_visible():
            return NotificationScreenResult.NONE
        if self.previous_button.handle_event(event):
            return NotificationScreenResult.PREVIOUS
        if self.current_button.handle_event(event):
            return NotificationScreenResult.CURRENT
        if self.close_button.handle_event(event):
            return NotificationScreenResult.CLOSE
        return NotificationScreenResult.NONE
...
__all__ = ["NotificationScreen", "NotificationScreenResult"]
